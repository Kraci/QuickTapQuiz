...
class HostPlayQuestionsViewModel(application: Application, quizGame: QuizGame): AndroidViewModel(application), HostPlayQuestionsListAdapter.ClickListener {
    ...
    init {
        ...
        for (category in quizGame.categories) {
            gameAdapter.add(GameAdapter(GameAdapterType.CATEGORY, category.name, "", "", 0, false))
            for (question in category.questions) {
                if (question.bonus) {
                    bonuses.put(category.name, question)
                } else {
                    gameAdapter.add(GameAdapter(GameAdapterType.QUESTION, question.text, question.hint, question.image, question.value, question.bonus, category.name))
                }
            }
        }
